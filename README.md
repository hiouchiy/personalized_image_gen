...
The third notebook, [03_fine_tuning](https://github.com/databricks-industry-solutions/personalized_image_gen/blob/main/03_fine_tuning.py), shows how to fine-tune Stable Diffusion XL using DreamBooth. Here, we combine techniques such as mixed precision and LoRA to make the training efficient and to reduce the memory footprint. In addition, we apply Databricks's DDP feature called TorchDistributor instead of original HF acceralate to optimize the distributed training for Databricks compute clusters both single and multi nodes. The second part of the notebook takes the fine-tuned model and registers it to Unity Catalog using MLflow. 
